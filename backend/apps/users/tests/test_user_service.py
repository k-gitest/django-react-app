...
class UserCommandServiceTestCase(TestCase):
    ...
    def test_create_user_with_extra_fields(self):
        ...
        user = self.service.create_user(
            email="extra@example.com", password="testpass123", is_staff=True
        )
    ...
    def test_create_user_duplicate_email(self):
        ...
        # Create first user
        self.service.create_user(email="duplicate@example.com", password="testpass123")

        # Attempt to create second user with same email
        with self.assertRaises(IntegrityError):
            self.service.create_user(
                email="duplicate@example.com", password="testpass456"
            )

    def test_create_user_password_is_hashed(self):
        """Test that password is properly hashed"""
        password = "testpass123"
        user = self.service.create_user(email="hashed@example.com", password=password)
    ...
    def test_create_user_with_adapter_duplicate_email(self):
        ...
        self.service.create_user_with_adapter(
            request=request, email="duplicate@example.com", password="testpass123"
        )

        # Attempt to create second user with same email
        with self.assertRaises(IntegrityError):
            self.service.create_user_with_adapter(
                request=request, email="duplicate@example.com", password="testpass456"
            )
    ...
    def test_update_user_multiple_fields(self):
        ...
        updated_user = self.service.update_user(
            user, first_name="Multi", last_name="Field", is_staff=True
        )
    ...
    def test_update_user_invalid_field_ignored(self):
        ...
        updated_user = self.service.update_user(
            user, first_name="Valid", invalid_field="Should be ignored"
        )
    ...
    def test_change_password_persists(self):
        ...
        user = User.objects.create_user(
            email="persist@example.com", password="oldpass123"
        )
    ...
    def test_delete_user_cascades(self):
        ...
        user = User.objects.create_user(
            email="cascade@example.com", password="testpass123"
        )

        # Create related todo
        todo = Todo.objects.create(user=user, todo_title="Test Todo")
        todo_id = todo.id
...
class UserRegistrationServiceTestCase(TestCase):
    ...
    def test_register_user_minimal_data(self):
        ...
        request = self.factory.post("/register")
        user_data = {"email": "minimal@example.com", "password": "testpass123"}
    ...
    def test_register_user_duplicate_email(self):
        ...
        user_data = {"email": "duplicate@example.com", "password": "testpass456"}
    ...
    def test_register_user_duplicate_email_case_insensitive(self):
        ...
        # Create first user
        User.objects.create_user(email="case@example.com", password="testpass123")

        # Attempt to register with same email (different case)
        user_data = {"email": "CASE@EXAMPLE.COM", "password": "testpass456"}
    ...
    def test_register_user_creates_in_database(self):
        ...
        request = self.factory.post("/register")
        user_data = {"email": "database@example.com", "password": "testpass123"}
    ...
    def test_register_user_uses_query_service(self):
        ...
        # Attempt to register with same email
        user_data = {"email": "existing@example.com", "password": "testpass456"}
    ...
    def test_register_user_password_not_stored_plaintext(self):
        ...
        request = self.factory.post("/register")
        user_data = {"email": "secure@example.com", "password": "testpass123"}
...
class UserServiceIntegrationTestCase(TestCase):
    ...
    def test_create_update_delete_flow(self):
        ...
        user = self.command_service.create_user(
            email="crud@example.com", password="testpass123"
        )
        ...
        updated_user = self.command_service.update_user(
            user, first_name="Updated", last_name="User"
        )
        self.assertEqual(updated_user.first_name, "Updated")
