...
class AuthenticationAPITest(TestCase):
    ...
    @patch("apps.users.views.AnalyticsService.log_auth_event")  # ← 追加
    ...
    def test_user_registration_with_duplicate_email(self):
        ...
        User.objects.create_user(email=duplicate_email, password="testpass123")
    ...
    @patch("apps.users.views.MotherDuckClient")
...
class AnalyticsEventWebhookTestCase(TestCase):
    ...
    @patch("apps.users.views.MotherDuckClient")
    def test_analytics_event_webhook_auth_event_success(
        self, mock_motherduck_class, mock_verify
    ):
    ...
    def test_analytics_event_webhook_unknown_event_type(self, mock_verify):
        ...
        # Arrange
        mock_verify.return_value = True
        payload = {"event_type": "unknown_event", "event_data": {}}
...
class DltPipelineWebhookTestCase(TestCase):
    ...
    @patch("apps.common.permissions.verify_qstash_signature")
    @patch("apps.users.views.subprocess.run")
