...
class TodoViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        todo = TodoService.update_todo(
            self.get_object().id, self.request.user, serializer.validated_data
        )
        # serializerのinstanceを設定（レスポンスに含めるため）
        serializer.instance = todo
    ...
    def search(self, request):
        ...
        results = TodoService.search_similar_todos(
            request.user, query, top_k=top_k, min_score=min_score
        )
...
def vector_indexing_webhook(request):
    ...
    if not todo_id:
        ...
        return Response(
            {"error": "todo_id is required"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def bulk_vector_indexing_webhook(request):
    ...
    try:
        ...
        if not todos:
            ...
            return Response({"message": "No todos to index", "count": 0})
