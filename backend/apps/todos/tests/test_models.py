...
class TodoModelTestCase(TestCase):
    ...
    def test_create_todo_with_default_values(self):
        """Todo作成: デフォルト値の確認"""
        todo = Todo.objects.create(user=self.user, todo_title="デフォルトタスク")
    ...
    def test_todo_str_representation(self):
        ...
        todo = Todo.objects.create(user=self.user, todo_title="文字列表現テスト")
    ...
    def test_user_cascade_delete(self):
        ...
        todo = Todo.objects.create(user=self.user, todo_title="削除されるタスク")
        todo_id = todo.id
    ...
    def test_priority_choices(self):
        ...
        for priority in [Todo.Priority.LOW, Todo.Priority.MEDIUM, Todo.Priority.HIGH]:
            todo = Todo.objects.create(
                user=self.user, todo_title=f"{priority}タスク", priority=priority
            )
    ...
    def test_todo_title_max_length(self):
        """todo_title: 最大255文字"""
        long_title = "あ" * 255
        todo = Todo.objects.create(user=self.user, todo_title=long_title)
    ...
    def test_auto_now_fields(self):
        ...
        todo = Todo.objects.create(user=self.user, todo_title="時刻テスト")
