...
from apps.todos.embedding_service import EmbeddingService
...
class EmbeddingServiceTestCase(TestCase):
    ...
    def test_prepare_text_basic(self):
        ...
        todo = Todo.objects.create(
            user=self.user, todo_title="会議資料の作成", priority="HIGH", progress=50
        )
    ...
    @patch("apps.todos.embedding_service.genai.embed_content")
    ...
    def test_prepare_text_empty_title(self):
        ...
        # Arrange
        todo = Todo.objects.create(
            user=self.user, todo_title="", priority="LOW", progress=100
        )
