...
class TodoServiceTestCase(TestCase):
    ...
    def test_create_todo_success(self):
        ...
        todo = TodoService.create_todo(self.user1, data)
    ...
    def test_create_todo_with_default_values(self):
        """create_todo: デフォルト値でタスクを作成"""
        data = {
            'todo_title': 'デフォルト優先度のタスク',
        }
    ...
    def test_update_todo_not_found(self):
        """update_todo: 存在しないタスクの更新は例外を発生"""
        update_data = {'progress': 100}
        
        with self.assertRaises(ObjectDoesNotExist):
            TodoService.update_todo(99999, self.user1, update_data)

    def test_update_todo_unauthorized_user(self):
        """update_todo: 他人のタスクは更新できない"""
        update_data = {'progress': 100}
        
        # user2がuser1のタスクを更新しようとする
        with self.assertRaises(ObjectDoesNotExist):
            TodoService.update_todo(self.todo1.id, self.user2, update_data)
