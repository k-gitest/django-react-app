...
import type { ClientConfig, InitOptions, RequestOptions, DataType } from '@/types/fetch-client-types';
...
export class FetchClient {
  ...
  private validateConfig(initOptions: InitOptions): ClientConfig {
    ...
    if (!result.success) {
      const errorMessages = result.error.issues.map((error) => `- ${error.message}`).join('\n');
